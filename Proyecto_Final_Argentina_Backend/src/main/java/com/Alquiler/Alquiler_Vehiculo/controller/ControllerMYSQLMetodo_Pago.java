...
public class ControllerMYSQLMetodo_Pago {
    ...
    public void updatePago(@PathVariable Long id, @RequestBody Metodo_PagoDTO metodoPagoDTO) {
        Optional<Metodo_PagoDTO> optionalMetodo = Optional.ofNullable(servicePago.findById(id));

        if (optionalMetodo.isPresent()) {
            Metodo_PagoDTO metodoEncontrado = optionalMetodo.get();
            metodoEncontrado.setMonto(metodoPagoDTO.getMonto());
            servicePago.save(metodoEncontrado);
        } else {
            throw new Excepciones("El método de pago no existe y no se puede actualizar", HttpStatus.NOT_FOUND);
        }
    }
}
